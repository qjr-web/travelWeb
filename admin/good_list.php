        	...
        	<div class="note">
                ...
                <table cellspacing="0" class="=news_list">
					...
					<tbody>
						...
						$sql="select g.* ,u.username ,s.title from good g,user u, story s where g.user_id=u.id and g.story_id=s.id order by intime limit $start,$pagesize";
						$rs=mysqli_query($conn,$sql);
                ...
                <div class="page">
					...
					for($i=1 ;$i<=$pagecount ;$i++){
						if($page==$i){
							echo"<a href='good_list.php?page=$i' class='on'>$i</a>";//给当前页的页码加上样式
						}else{
							echo"<a href='good_list.php?page=$i'>$i</a>";
						}
					}
					?>
                </div>
