        	...
        	<div class="note">
                ...
                <table cellspacing="0" class="=news_list">
					...
					<tbody>
						...
						$sql="select c.* ,u.username ,s.title from comments c,user u, story s where c.user_id=u.id and c.story_id=s.id order by intime limit $start,$pagesize";
						$rs=mysqli_query($conn,$sql);
						while($row=mysqli_fetch_assoc($rs)){
							...
							echo'<td>';
							echo'<a href="comment_delete.php?id='.$row['id'].'" onclick="return confirm(\'你确定要删除该数据吗？\')">删除</a>';
                ...
                <div class="page">
					...
					for($i=1 ;$i<=$pagecount ;$i++){
						if($page==$i){
							echo"<a href='comment_list.php?page=$i' class='on'>$i</a>";//给当前页的页码加上样式
						}else{
							echo"<a href='comment_list.php?page=$i'>$i</a>";
						}
					}
					?>
                </div>
